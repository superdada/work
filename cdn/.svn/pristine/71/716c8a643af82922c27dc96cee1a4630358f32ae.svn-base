...
public class RealTimeOnlineServiceImpl implements RealTimeOnlineService{
    ...
    public RealTimeOnlineData1 handler1(Map<String,String> params) {
        ...
        String querySQL="SELECT * FROM shcdn.r_realtime_10000 where "+cdn_cache_all_Condition+"  ORDER BY ttime DESC limit 1";
        ...
        if(null != map) {
            ...
            realTimeOnlineData1 = new RealTimeOnlineData1(
                    ...
                    TimeUtil.TextToMinDateText(ttime.toString()),
...
//    @Override
//    public RealTimeOnlineData2 handler2(String index, String IPtype) {
//        switch (IPtype) {
//            case "data_center":
//                Map<String,Object> data_center = jdbcTemplateVertica.queryForMap(String.format("SELECT %s as time FROM %s where (cache_cdn='ALL' or cache_cdn is null)  order by ttime desc limit 1", "ttime", "shcdn.r_realtime_11000"));
//                return new RealTimeOnlineData2(TimeUtil.TextToMinDateText(data_center.get("time").toString()),
//                        jdbcTemplateVertica.queryForList(String.format("SELECT %s as name, %s as value FROM %s where ttime = cast('%s' as timestamp) and (cache_cdn='ALL' or cache_cdn is null)  order by %s",
//                                "dc_city", index, "shcdn.r_realtime_11000", data_center.get("time").toString(), "dc_city"))// 数据中心位置表
//                ); // 用户位置表
//            case "user":
//                Map<String,Object> user = jdbcTemplateVertica.queryForMap(String.format("SELECT %s as time FROM %s where user_province='广东' and (cache_cdn='ALL' or cache_cdn is null) and user_city is not null order by %s desc limit 1", "ttime", "shcdn.r_realtime_10010", "ttime"));
//                return new RealTimeOnlineData2(TimeUtil.TextToMinDateText(user.get("time").toString()),
//                    jdbcTemplateVertica.queryForList(String.format("SELECT %s as name, %s as value FROM %s where user_province='广东' AND ttime = cast('%s' as timestamp) and (cache_cdn='ALL' or cache_cdn is null) and user_city is not null order by %s",
//                        "user_city", index, "shcdn.r_realtime_10010", user.get("time").toString(), "user_city"))
//                ); // 用户位置表
//        }
//        return null;
//    }
    ...
    public RealTimeOnlineData2 handler2(Map<String, String> params) {
        ...
        String cache_cdn_condition=params.get("cache_cdn")==null||"ALL".equals(params.get("cache_cdn").toUpperCase())?" cache_cdn in('ALL',NULL)":" cache_cdn='"+params.get("cache_cdn")+"'";

        switch (IPtype) {
            case "data_center":
                Map<String,Object> data_center = jdbcTemplateVertica.queryForMap(String.format("SELECT %s as time FROM %s where "+cache_cdn_condition+"  order by ttime desc limit 1", "ttime", "shcdn.r_realtime_11000"));
                return new RealTimeOnlineData2(TimeUtil.TextToMinDateText(data_center.get("time").toString()),
            ...
            case "user":
                String sql="SELECT %s as time FROM %s where user_province='广东' and "+cache_cdn_condition+" and user_city is not null order by %s desc limit 1";
                logger.info(sql);
                sql=String.format(sql, "ttime", "shcdn.r_realtime_10010", "ttime");
                logger.info(sql);
                Map<String,Object> user = jdbcTemplateVertica.queryForMap(sql);
                return new RealTimeOnlineData2(TimeUtil.TextToMinDateText(user.get("time").toString()),
                        jdbcTemplateVertica.queryForList(String.format("SELECT %s as name, %s as value FROM %s where user_province='广东' AND ttime = cast('%s' as timestamp) and "+cache_cdn_condition+" and user_city is not null order by %s",
                                "user_city", index, "shcdn.r_realtime_10010", user.get("time").toString(), "user_city"))
                ); // 用户位置表
        }
        return null;
    ...
    public RealTimeOnlineData3 handler3(Map<String, String> params) {
        ...
        String cache_cdn_condition=params.get("cache_cdn")==null||"ALL".equals(params.get("cache_cdn"))?" cache_cdn in('ALL',NULL)":" cache_cdn='"+params.get("cache_cdn")+"'";

        String querySQL=String.format("SELECT %s as time FROM %s where "+cache_cdn_condition+" order by ttime desc limit 1", "ttime", "shcdn.r_realtime_10000");
        logger.info(querySQL);
        ...
        List<Map<String,Object>> data = jdbcTemplateVertica.queryForList(String.format("SELECT ttime as x, %s as y FROM shcdn.r_realtime_10000 where %s and "+cache_cdn_condition+"  order by ttime ", index, timeWhere));
