...
function initVM() {
    var vm = new Vue({
        ...
        data: {
            ...
            dimensions1 : [{'type':'data_center', 'text': '内容中心'},{'type':'fringe_node', 'text': '边缘节点'},{'type':'ICP', 'text': 'ICP'},{'type':'service_type','text':'业务类型'}],
        ...
        computed:{
            ...
            items  : function () {
                ...
                this.$nextTick(function(){
                    if(this.curr_dimension !="service_type" && this.directory_level == 1){
                        $(".my-list-items").children("li").children().each(function(index,element){
                            $(element).children("li:first").addClass("textline");
                        });
                    }
        ...
        methods : {
            ...
            moreinfo : function(_item, _index ) {
                if(_index.type != null){
                    $(window).trigger('event_moreinfo', [ vm.$data.curr_datasource,vm.$data.curr_cdn,vm.$data.curr_dimension, _item, _index.time,vm.$data.directory_level]);
                } else
                    return 0;
            },
//                moreinfo : function(_item, _index ) {
...
//                },
                ...
                if (_type=='datasource') {
                    ...
                    layui.use('form', function() {
                        ...
                        if (_option == 'cdn') {
                           ...
                           $("#dimension").empty();
                            $.each(vm.$data.dimensions1, function (n, value) {
                                if(vm.$data.curr_dimension == value.type){
                            ...
                            if(vm.$data.curr_dimension == "data_center"){
                                $("#dimension").find("option").eq(0).attr("selected","selected");
                                vm.$data.curr_dimension =$("#dimension").find("option").eq(0).val();
                            }
                        }
                        form.render('select');
                    });
            ...
            changeState : function(_type, _id, _monitorFlag, _alarmFlag, _index) {
                ...
                $.post(AppBase+'/pfcsnapshot.do?method=request3',{
                    ...
                    'datasource':vm.$data.curr_datasource,
                    'dimension' : vm.$data.curr_dimension,//当前维度
                    'cache_cdn':vm.$data.curr_cdn,
                    'directory_level':vm.$data.directory_level,
                    'id' : _id//目标id
                },function (Response) {
                  //重新请求数据：
                    this.refreshpage();
                ...
                $.post(AppBase+'/pfcsnapshot.do?method=request5',{ // 分享链接
                    ...
                    function showMsg(Response){
                        Ext.Msg.show({
                            ...
                            prompt:true,
                            multiline: false,
                            value: (location.href.substr(0, location.href.indexOf('web'))) + Response.data,
                            fn: function (btn) {
                                if (btn == 'ok') {
