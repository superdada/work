...
<div id="vm" >
    <div class="my-switch-box">
        <div class="my-switch-item" style="width: 310px">
        ...
        <div class="my-switch-item"style="width: 380px">
        ...
        <div class="my-switch-item">
            ...
            <ul class="my-switch-options">
                <li v-for="serverType in serverTypes" v-on:click="switch('serverType',serverType.type)" v-bind:class="{ 'my-options-on': curr_serverType == serverType.type, 'my-options-off': curr_serverType != serverType.type}">{{serverType.text}}</li>
            </ul>
        </div>
...
<script type="text/javascript">
    $(function () {
        /*初始化页面vm*/
        $.getJSON(AppBase + '/pfcsnapshot.do?method=request1', {'max_rows': 10,'curr_page' : 1, 'dimension' : 'data_center', 'index' : 'healthy'}, function (Response) {var data = Response.data;
            ...
            if(_data.length <= _max_rows ) {
                ...
                for(var d = 1; d <= _data.length/_max_rows ; d++) {
                    var _li = [];
                    for(var j=1; j < _max_rows+1 ; j++){
                        _li.push(_data[d*j-1]);
                    };
                    _vmData["page"+d] = _li;
                }
            };
    ...
    function initVM(_itemsData) {
        var vm = new Vue({
            ...
            data: {
                /*查询条件相关配置*/
                dimensions : [{'type':'data_center', 'text': '数据中心'},{'type':'ICP', 'text': 'ICP'},{'type':'service', 'text': '业务类型'}],
                indexes : [{'type':'healthy', 'text': '健康度'},{'type':'hit_rate', 'text': '命中率'},{'type':'data_rate', 'text': '速率'},{'type':'delay', 'text': '时延'}],
                serverTypes : [{'type':'all', 'text': 'ALL'},{'type':'cdn', 'text': 'CDN'},{'type':'cache', 'text': 'Cache'}],
                curr_dimension : 'data_center',//当前维度类型，枚举值{'data_center', 'ICP'}
                curr_index : 'healthy',//当前指标类型，枚举值{'healthy', 'hit_rate', 'data_rate'}
                curr_serverType : 'all',//当前指标类型，枚举值{'all', 'CDN', 'Cache'}
                /*信息窗口相关配置*/
            ...
            computed:{
                ...
                list_titles : function () {
                    ...
                    if(this.curr_dimension =='data_center') {
                        ...
                        _list_titles = ['域名', 'ICP', '最近24小时状态', '最近监测时间'];
                    } else if (this.curr_dimension=='service') {
            ...
            methods : {
                ...
                switch : function(_type, _option) {
                    ...
                    } else if (_type=='serverType') {
                        this.curr_serverType = _option;
                    } else
                        return 0;
                    $.getJSON(AppBase + '/pfcsnapshot.do?method=request1', { 'dimension' : this.curr_dimension, 'index' : this.curr_index, 'serverType' : this.curr_serverType}, function (Response) {
                        var _data = Response.data;
                        var _max_rows = 10;
                        var _vmData = {};
                        if(_data.length <= _max_rows ) {
                    ...
                    $.post(AppBase+'/pfcsnapshot.do?method=request5',{ // 分享链接
                        ...
                        function showMsg(Response){
                            Ext.Msg.show({
                                ...
                                prompt:true,
                                multiline: false,
                                value: (~location.href.indexOf('?') ? location.href.substr(0, location.href.indexOf('?')) : location.href) + Response.data,
                                fn: function (btn) {
                                    if (btn == 'ok') {
                ...
                download : function (_item) {
                    $.post(AppBase+'/pfcsnapshot.do?method=request6',{ // 查询数据
                        ...
                        Ext.MessageBox.confirm(Response.msg, Ext.String.format(" [ {0} ] [ {1} ] ", _item.name, _item.belong_to) + Response.data, function(btn){
                            if(btn == 'yes'){
                ...
                changePage : function(_type) {
                    if (_type=='Previous') {
                        ...
                        var _pageNum =  $("input[id='my-page-num']").val();
                        var re = /^\+?[1-9][0-9]*$/;　　//正整数
